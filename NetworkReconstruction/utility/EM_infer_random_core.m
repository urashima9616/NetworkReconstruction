...
Z_samples_name =sprintf('graph_samples_random_%d_loss_%d_nodes_alpha_%d.mat', time_steps, N, alpha);
LL_samples_name =sprintf('LL_samples_random_%d_loss_%d_nodes_alpha_%d.mat', time_steps, N, alpha);
LL_trace_name = sprintf('LL_trace_random_%d_loss_%d_nodes_alpha_%d.mat', time_steps, N, alpha);
P_est_name = sprintf('P_est_random_%d_loss_%d_nodes_alpha_%d.mat', time_steps, N, alpha);
sequence_name = sprintf('sequence_random_%d_loss_%d_nodes_alpha_%d.mat', time_steps, N, alpha);
...
while batch <= N_batches
    ...
    while iter < N_opt_steps
        ...
        for s = 1: N_samples
            ...
            LL_sample(s, batch) = LL_calc(A_obs, P_k_est, psi); %+ attack_sequence_prob;
            G_sample = graph(A_obs);
            cur_gradient = gradient_calc_v2(P_est, P_k_est, m_opt, k_opt,  G_sample, psi);
