...
%load the network:
%Load the estimated G6 network
load('G6.mat')

adjacency_matrix = full(adjacency(G6));
...
Z_samples_name =sprintf('facebook_graph_samples_random_%d_loss_%d_nodes_alpha_%d.mat', time_steps, N, alpha);
LL_samples_name =sprintf('facebook_LL_samples_random_%d_loss_%d_nodes_alpha_%d.mat', time_steps, N, alpha);
LL_trace_name = sprintf('facebook_LL_trace_random_%d_loss_%d_nodes_alpha_%d.mat', time_steps, N, alpha);
P_est_sequence_name = sprintf('facebook_P_est_sequence_random_%d_loss_%d_nodes_alpha_%d.mat', time_steps, N, alpha);
P_est_name = sprintf('facebook_P_est_random_%d_loss_%d_nodes_alpha_%d.mat', time_steps, N, alpha);
attack_prob_name = sprintf('facebook_random_prob_attack_%d_loss_%d_nodes_alpha_%d.mat', time_steps, N, alpha);
sequence_name = sprintf('facebook_sequence_random_%d_loss_%d_nodes_alpha_%d.mat', time_steps, N, alpha);
psi_name = sprintf('facebook_psi_sequence_random_%d_loss_%d_nodes_alpha_%d.mat', time_steps, N, alpha);
