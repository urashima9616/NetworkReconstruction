...
A_obs = adjacency(G);
iter = 0;
momentum = zeros(size(P_est));
rms_prop = zeros(size(P_est));
...
while 1
    ...
    for kk = 1:Burnin+N_samples
        ...
        cur_gradient = gradient_calc_v2(P_est, P_k_est, m_opt, k_opt,  G, psi);
    ...
    %momentum = momentum./(1-beta);
    %P_est_test = P_est + lambda.*(momentum./(rms_prop.^(1/2)+ones(size(P_est)).*epsilon));
