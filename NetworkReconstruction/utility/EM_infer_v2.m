...
while 1
    ...
    for kk = 1:Burnin+N_samples
        ...
        if use_correct_order == 0 %Linear scanning
            [A_obs, index_array_sample, attack_sequence_prob, flag] = rejection_sampling_single_v2(A_obs, P_k, psi, 1, sequence_missing, alpha, index_array_sample);
      
        else % Sanity check. Always infer using the correct ordering
            [A_obs, index_array_sample, attack_sequence_prob, flag] = rejection_sampling_single_v2(A_obs, P_k, psi, 1, fliplr(sequence), alpha, index_array_sample);
