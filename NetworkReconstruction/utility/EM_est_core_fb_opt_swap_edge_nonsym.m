        ...
        if s >= 1 + Burnin
            ...
            if s == 1 + Burnin
                ...
                if changed
                    ...
                    cur_gradient = update_gradient_nonsym(N, A_obs, P_est, P_k_est, labels, cur_gradient, idx_j, idx_k, psi, psi_new);
                    %Debug use
                    %test_gradient = gradient_calc_opt_nonsym(P_est, P_k_est, m_opt, k_opt,  G, psi_new, labels);
                    %num_gradient = num_grad_calc(A_obs, P_est, P_k_est, psi_new, m_opt, k_opt);
...
%             
    ...
    fprintf("(2,1): %f + %f -> %f\t Gradient: %f \n", P_est(2,1),delta_P(2,1), P_est_test(2,1), momentum(2,1));
