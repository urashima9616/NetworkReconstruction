function [G_new, sequence, index_array, max_sequence, partition_all] = attack_model_trans(G, alpha, time_steps)
A = full(adjacency(G));
[N, m] = size(A);
index_array = ones(1,N);
degree_array = zeros(1, N);
sequence = zeros(1, time_steps);
partition_all = zeros(time_steps, N);
for i = 1:N
