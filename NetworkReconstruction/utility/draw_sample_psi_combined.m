function [new_psi, changed, LL_new, idx_j, idx_k] = draw_sample_psi_combined(P_k_est, N, LL_prev, A_obs, psi, edgelist, len, omega)
%Step 1: draw two sample of index
changed = 0;
dice = rand(1);
if dice > omega
    edge = floor(len*rand(1))+1;
    idx_j = psi(edgelist(edge, 1));
    idx_k = psi(edgelist(edge, 2));
else
